		...
		for (TextBlock tb : doc.getTextBlocks()) {
			...
			if (numWords < 15) {
				...
				if (len >= 8) {
					...
					if (textLC.startsWith("comentários")
							|| startsWithNumber(textLC, len, " comentários",
									" usuários responderam", " visualizações")
							|| textLC.startsWith("© reuters")
							|| textLC.startsWith("dê uma nota")
							|| textLC.startsWith("poste um comentário")
							|| textLC.contains("O que você está pensando...")
							|| textLC.contains("adicione seu comentário")
							|| textLC.contains("adicionar comentário")

							|| textLC
									.equals("Obrigado pelos comentários")) {
						tb.addLabel(DefaultLabels.INDICATES_END_OF_TEXT);
						changes = true;
					}
				} else if(tb.getLinkDensity() == 1.0) {
					if(text.equals("Comentário")) {
						tb.addLabel(DefaultLabels.INDICATES_END_OF_TEXT);
					}
				}
			}
